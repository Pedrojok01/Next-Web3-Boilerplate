...
const TOKEN = process.env.USERS_REST_API_TOKEN ?? "AZ0IASQgNzI2ZTljMjAtNjBhZC00MWYyLTk1MTYtZGVlYTk2ZDM0Zjk1YTgxNmI3OWYzYmZlNDMxZDgyMGY3YzIyZTBkODg3YmM=";
...
export type Persistence = {
   ...
   * @param page
   */
  list: (namespace: string, page?: { start: number, limit: number }) => Promise<Record<string, unknown>>
  /**
   * find list
   * @param namespace
   * @param page
...
class KvPersistence implements Persistence {
  ...
  async list(namespace: string, page?: { start: number; limit: number }): Promise<Record<string, unknown>> {
    console.log(page);
