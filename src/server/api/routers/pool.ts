...
export const poolRouter = createTRPCRouter({
  ...
  initPool: publicProcedure
    .mutation(async (): Promise<ResponseTPRC> => {
      try {
        const power: Lottery = { name: "PowerBlast", poolHash: nanoid(), difficulty: Difficulty.MATCH, period: 100000 };
        const buzz: Lottery = { name: "Buzz", poolHash: nanoid(), difficulty: Difficulty.MATCH, period: 10000 };
        const jolt: Lottery = { name: "Jolt", poolHash: nanoid(), difficulty: Difficulty.MATCH, period: 1000 };
  ...
  poolList: publicProcedure
    .query(async (): Promise<ResponseTPRC> => {
      try {
